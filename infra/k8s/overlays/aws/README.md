...
`auth-api-deploy.yaml` workflow uses `kubectl set image` with the git SHA. It bypasses this kustomization's `newTag` at runtime — that's intentional and fine. No file changes needed in git.
